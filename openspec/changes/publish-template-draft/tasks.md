...
- [ ] 1.1 Escribir `apps/api/drizzle/0025_publish_template_from_builder.sql` §1:
      ...
      sin `DELETE`.
- [ ] 1.2 §2: `ALTER TABLE template_draft` agrega `published_at timestamptz` y
...
- [ ] 1.3 §3: reescribir entera la lista de `GRANT UPDATE` por columna sobre `template_draft`
...
- [ ] 1.4 §4: recrear `template_draft_key_live_idx` (`0016` §4) y `template_draft_name_live_idx`
      (`0017` §1) con `WHERE discarded_at IS NULL AND published_at IS NULL`.
- [ ] 1.5 Reflejar las dos columnas a mano en `apps/api/src/db/schema/templates.ts`
      (`drizzle-kit generate` está prohibido, ADR-004).

## 2. Contratos

- [ ] 2.1 En `packages/contracts/src/templates.ts`: `publishedTemplateSchema`
...
- [ ] 2.2 Actualizar la cabecera del archivo, que hoy dice que la publicación queda fuera.
- [ ] 2.3 Test de `packages/contracts` para el esquema nuevo, junto a los que ya hay.

## 3. API

- [ ] 3.1 `templates.errors.ts`: agregar `template_draft_not_publishable` (409, con `issues` en
...
- [ ] 3.2 `templates.repository.ts`: `findDraft` y `findDrafts` suman `published_at IS NULL`;
...
- [ ] 3.3 `templates.service.ts`: `publishDraft(session, id)` — rol, borrador vivo,
      ...
      `withSiteScope`.
- [ ] 3.4 `templates.controller.ts`: `POST drafts/:id/publish`, y corregir el docblock que hoy
      afirma que publicar no vive en ningún endpoint. Lo mismo en `templates.module.ts`.
- [ ] 3.5 Verificar que la transacción de `DbService` envuelve las cuatro escrituras y que un
      rechazo tardío no deja `template` sin versión.

## 4. Web

- [ ] 4.1 `apps/web/src/api/templates.ts`: `publishTemplateDraft(id)` contra
      `POST /templates/drafts/:id/publish`, parseando `publishedTemplateSchema`.
- [ ] 4.2 `apps/web/src/permissions/session.ts`: `canPublishTemplates`, con su test —una función
      por decisión, aunque hoy devuelva lo mismo que `canAuthorTemplates`.
- [ ] 4.3 `TemplateDraftRoute/PublishDialog.tsx`: la confirmación, modelada sobre
...
- [ ] 4.4 `TemplateDraftRoute/DraftHeader.tsx`: el botón Publish al lado de Save, habilitado solo
...
- [ ] 4.5 `TemplateDraftRoute/index.tsx`: la mutación, el diálogo montado desde la ruta, la
      invalidación de `templateDrafts()` **y** `templates()`, y la navegación a `/templates` al
      publicar.
- [ ] 4.6 Reescribir el texto de `PublishReadiness.tsx` ("once publishing is available") y el
      `notice-card` de `TemplatesRoute/TemplateDrafts.tsx` ("not available yet").

## 5. Tests

- [ ] 5.1 `apps/api/test/templates.int-spec.ts`: publicación completa (filas, proyección,
...
- [ ] 5.2 Test de integración de que un borrador publicado libera su nombre para uno nuevo.
- [ ] 5.3 `apps/web/src/routes/TemplateDraftRoute/index.test.tsx`: el botón deshabilitado con
      issues o con cambios sin guardar, el diálogo, y la publicación exitosa.
- [ ] 5.4 `presentation.test.ts` para las funciones puras nuevas.

## 6. Cierre

- [ ] 6.1 `pnpm -r build && pnpm typecheck && pnpm lint`, `pnpm test` y
      `pnpm --filter api test:int`.
- [ ] 6.2 `openspec validate publish-template-draft --strict`.
- [ ] 6.3 Comprobar contra `openspec/specs/templates/spec.md` que ningún requisito que queda en
