...
export function SchedulingRoute(): React.JSX.Element {
  ...
  const siteName = (id: string): string =>
    sites.data?.find((site) => site.id === id)?.name ?? "";
  ...
  const ready =
    ...
    scheduled.isSuccess &&
    siteId !== "";
  const rules = (schedules.data ?? []).filter(
    (rule) => rule.site_id === siteId,
  );
  const periods = (scheduled.data ?? []).filter(
    (entry) => entry.site_id === siteId,
  );
  const entries = ready ? projectYear(rules, periods, year) : [];
  const yearUnassigned = entries.filter(
    (entry): entry is Extract<YearEntry, { kind: "opened" }> =>
      entry.kind === "opened" && isUnassigned(entry.inspection),
  );
  const notice = unassignedNotice(
    yearUnassigned.map((entry) => entry.inspection),
  );
  ...
  return (
    <>
      <header className="scheduling__top roster-header">
        <div className="scheduling__header">
          ...
          <p className="scheduling__subtitle">
            Plan and assign each inspection period this site owes throughout the
            year.
          </p>
        </div>
      ...
      {sites.isLoading || schedules.isLoading || scheduled.isLoading ? (
        <p className="status-card">
          <CalendarIcon size={20} /> Loading scheduling data…
        </p>
      ) : null}
      {sites.isError || schedules.isError || scheduled.isError ? (
        <p className="status-card status-card--error">
          <InfoIcon size={20} /> This view needs a connection. Scheduling data
          could not be loaded.
        </p>
      ) : null}
      {noActiveSite ? (
        ...
        <div className="notice-card">
          <div className="notice-card__body">
            ...
            <p className="notice-card__text">
              {notice} This notice is limited to {year}.
            </p>
          </div>
        </div>
      ) : null}
