...
- [x] 1.4 En la misma función, un bloque `IF TG_TABLE_NAME = 'scheduled_inspection' AND
  new_row ->> 'template_version_id' IS DISTINCT FROM old_row ->> 'template_version_id'`
      ...
      `HINT` que diga qué hacer en su lugar: 1. la `version` de la nueva tiene que ser estrictamente mayor que la de la vieja
      (subconsulta a `template_version`; la FK compuesta ya garantiza la plantilla, y el
      comentario tiene que decirlo para que nadie agregue la comprobación de más); 2. `NOT EXISTS (SELECT 1 FROM inspection WHERE scheduled_inspection_id = OLD.id)`; 3. `old_row ->> 'cancelled_at' IS NULL`.
...
- [x] 6.2 A mano: publicar una revisión con un período abierto y sin borrador → _Refresh
      field package_ → el héroe pasa a la versión nueva y _Start inspection_ la usa. Repetir
      con un borrador en curso → no avanza y aparece el aviso de descartar.
- [x] 6.3 `/opsx:archive` con la fecha del día.
