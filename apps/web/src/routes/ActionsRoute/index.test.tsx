...
describe('ActionsRoute', () => {
  ...
  it('agrupa por inspección y enlaza a la vista de esa inspección', async () => {
    const manual = action({
      ...
      source: { kind: 'manual_finding', finding_id: FINDING_A },
    });
  ...
  /**
   * La creación se mudó a `/findings/$id`, donde el hallazgo se lee junto a la pregunta que
   * lo abrió. Se comprueba la AUSENCIA porque es la decisión del change: esta pantalla ya no
   * mezcla dos recursos, y un botón que volviera sin querer la volvería a mezclar.
   */
  it('no ofrece crear acciones ni lista hallazgos', async () => {
    listActions.mockResolvedValue([action()]);

    renderRoute();

    await screen.findByText('Monthly walkthrough');
