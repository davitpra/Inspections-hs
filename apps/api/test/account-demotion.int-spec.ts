...
const demotion = updateAccountRequestSchema.parse({ demote_to: 'jhsc_member' });
const asManager = () => ({
...
describe('degradación de una cuenta a jhsc_member', () => {
  it('management degrada una cuenta de dos sitios y audita ambos roles en cada cadena', async () => {
    ...
    for (const siteId of [SITE_A, SITE_B]) {
      ...
      expect(entry?.payload['previous_role']).toBe('hs_coordinator');
      expect(entry?.payload['role']).toBe('jhsc_member');
    }
  });
  ...
  it('conserva credencial y sesión, pero la siguiente lectura pierde administración', async () => {
    ...
    expect(resolved.role).toBe('jhsc_member');
  ...
  it('aplica las guardas de actor, rol actual, actividad, alcance y auto-degradación', async () => {
    const coordinator = await createAccount(db.app, { role: 'hs_coordinator', siteIds: [SITE_A] });
    ...
    expect(await roleOf(coordinatorTarget.accountId)).toBe('hs_coordinator');
    expect(
      await codeOf(() =>
        accounts.update(
    ...
    const inactive = await createAccount(db.app, { role: 'hs_coordinator', siteIds: [SITE_A] });
    ...
    expect(await roleOf(inactive.accountId)).toBe('hs_coordinator');

    const outside = await createAccount(db.app, { role: 'hs_coordinator', siteIds: [SITE_B] });
    expect(
      await codeOf(() =>
        accounts.update(
    ...
    expect(await roleOf(outside.accountId)).toBe('hs_coordinator');
  ...
  it('promover y degradar la misma cuenta deja dos cambios de rol por sitio', async () => {
    const target = await createAccount(db.app, { role: 'jhsc_member', siteIds: [SITE_A, SITE_B] });
    ...
    for (const siteId of [SITE_A, SITE_B]) {
      ...
      expect(entries.map((row) => [row.payload['previous_role'], row.payload['role']])).toEqual([
        ['jhsc_member', 'hs_coordinator'],
        ['hs_coordinator', 'jhsc_member'],
      ]);
    }
  });
});
